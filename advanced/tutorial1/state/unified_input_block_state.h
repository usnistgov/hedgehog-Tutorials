...
#ifndef HEDGEHOG_TUTORIALS_UNIFIED_INPUT_BLOCK_STATE_H
#define HEDGEHOG_TUTORIALS_UNIFIED_INPUT_BLOCK_STATE_H
...
> {
 private:
  ...
  std::vector<std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>>>
      gridMatrixA_ = {};

  std::vector<std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>>>
      gridMatrixB_ = {};
 ...
 public:
  CudaInputBlockState(size_t gridHeightLeft, size_t gridSharedDimension, size_t gridWidthRight) :
    ...
    gridMatrixA_ = std::vector<std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>>>
        (gridHeightLeft_ * gridSharedDimension_, nullptr);
    gridMatrixB_ = std::vector<std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>>>
  ...
  void execute([[maybe_unused]]std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>> ptr) override {
    matrixA(ptr);
    for (size_t jB = 0; jB < gridWidthRight_; ++jB) {
      if (auto bB = matrixB(ptr->colIdx(), jB)) {
        ...
        auto res = std::make_shared<std::pair<std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>>,
  ...
  void execute([[maybe_unused]]std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>> ptr) override {
    matrixB(ptr);
 ...
 private:
  std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>> matrixA(size_t i, size_t j) {
    std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>> res = nullptr;
  ...
  std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>> matrixB(size_t i, size_t j) {
    std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>> res = nullptr;
  ...
  void matrixA(std::shared_ptr<UnifiedMatrixBlockData<Type, 'a'>> blockA) {
    gridMatrixA_[blockA->rowIdx() * gridSharedDimension_ + blockA->colIdx()] = blockA;
  }

  void matrixB(std::shared_ptr<UnifiedMatrixBlockData<Type, 'b'>> blockB) {
...
#endif //HEDGEHOG_TUTORIALS_UNIFIED_INPUT_BLOCK_STATE_H
